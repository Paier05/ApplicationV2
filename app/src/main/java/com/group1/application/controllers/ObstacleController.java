...
public class ObstacleController {
    ...
    private int batchDragStartRow = -1;
    private int batchDragStartCol = -1;
    ...
    public void setUIComponents(Button addObstacleButton, Button removeObstacleButton, Button clearAllObstaclesButton,
        ...
        this.removeObstacleButton = removeObstacleButton;
    ...
    private void setupClickListeners() {
        ...
        if (removeObstacleButton != null) {
            removeObstacleButton.setOnClickListener(v -> setCurrentObstacleAction("remove"));
        }
    ...
    private void setupActionUI(String action) {
        if ("add".equals(action)) {
            ...
            if (borderDirectionSection != null) borderDirectionSection.setVisibility(View.GONE);
            if (obstacleActionStatus != null) obstacleActionStatus.setText("Click on an obstacle to remove it");
            if (confirmObstacleButton != null) confirmObstacleButton.setVisibility(View.GONE);
            if (cancelObstacleButton != null) cancelObstacleButton.setVisibility(View.GONE);
            if (clearAllObstaclesButton != null) clearAllObstaclesButton.setVisibility(View.VISIBLE);
            isDraggingObstacle = false;
            showToast("Remove mode: Click on an obstacle to remove it");
        } else {
            if (borderDirectionSection != null) borderDirectionSection.setVisibility(View.GONE);
    ...
    public void handleObstacleClick(int row, int col) {
        ...
        if (currentObstacleAction.isEmpty()) {
            showToast("Please select Add or Remove first");
            return;
        }
        ...
        switch (currentObstacleAction) {
            case "add":
                ...
                return;
            case "remove":
                handleRemoveObstacleClick(row, col);
                return;
            default:
        }
    }
    ...
    private void handleRemoveObstacleClick(int row, int col) {
        if (gridAdapter.isCellObstacle(row, col)) {
            gridAdapter.setObstacle(row, col, false);
            showToast("Obstacle removed");
        } else {
            showToast("No obstacle to remove at this cell");
        }
    }
    ...
    public int getBatchDragStartRow() { return batchDragStartRow; }
    public int getBatchDragStartCol() { return batchDragStartCol; }
    ...
    private String getDirectionFromDialog(int which) {
        switch (which) {
            case 0:
                return "N";
            case 1:
    ...
    public void addObstacleBatch(int startRow, int startCol, int endRow, int endCol, String direction) {
        ...
        if (minRow == maxRow) { // Horizontal drag
            if (maxCol - minCol < 8) {
                for (int i = minCol; i <= maxCol; i++) {
                    gridAdapter.setObstacle(minRow, i, true);
                    gridAdapter.highlightCellBorder(minRow, i, Color.parseColor("#4CAF50"), direction);
                }
            }
        } else if (minCol == maxCol) { // Vertical drag
            if (maxRow - minRow < 8) {
                for (int i = minRow; i <= maxRow; i++) {
                    gridAdapter.setObstacle(i, minCol, true);
                    gridAdapter.highlightCellBorder(i, minCol, Color.parseColor("#4CAF50"), direction);
                }
            }
        }
    }

    public void setBatchDragStart(int row, int col) {
        batchDragStartRow = row;
        batchDragStartCol = col;
    }
    ...
    private void placeTempObstacleByCoordinates() {
        ...
        try {
            ...
            // Validate coordinate ranges (X: 0-190, Y: 0-190)
            if (displayX < 0 || displayX > 190) {
                updateTempObstacleStatus("X coordinate must be between 0 and 190", false);
                return;
            }
            if (displayY < 0 || displayY > 190) {
                updateTempObstacleStatus("Y coordinate must be between 0 and 190", false);
                return;
            }

            // Convert display coordinates to grid coordinates
            int gridCol = (displayX / 10) + 1;
            int gridRow = (gridAdapter.getGridSize() - 2) - (displayY / 10);
