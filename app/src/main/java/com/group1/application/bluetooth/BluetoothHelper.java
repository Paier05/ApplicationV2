package com.group1.application.bluetooth;
...
public class BluetoothHelper {
    ...
    private volatile boolean isListeningForConnection = false;
    ...
    private final Runnable continuousReconnectRunnable = new Runnable() {
        @Override
        public void run() {
            if (!continuousReconnectEnabled || isConnected()) {
                Log.d(TAG, "[ContinuousReconnect] Stopping reconnect task (disabled, connected, or no device).");
                return;
            }
            if (lastConnectedDevice != null) {
                Log.d(TAG, "[ContinuousReconnect] Attempting to connect to " + lastConnectedDevice.getAddress());
    ...
    public void connectToDevice(BluetoothDevice device) {
        new Thread(() -> {
            try {
                ...
                // If continuous reconnect is enabled, schedule the next attempt.
                if (continuousReconnectEnabled && lastConnectedDevice != null) {
                    Log.d(TAG, "Scheduling next reconnect attempt in " + continuousReconnectIntervalMs + "ms");
                    reconnectionHandler.removeCallbacks(continuousReconnectRunnable); // Ensure no duplicates
                    reconnectionHandler.postDelayed(continuousReconnectRunnable, continuousReconnectIntervalMs);
                } else {
                    // Fallback for non-continuous mode or if it's the first failed attempt.
                    new Handler(Looper.getMainLooper()).post(() -> {
                        if (connectionListener != null) {
        ...
        serverThread = new Thread(() -> {
            try {
                ...
                if (!isListeningForConnection) {
                    ...
                    Log.e(TAG, "Unexpected server socket error", e);

                    new Handler(Looper.getMainLooper()).post(() -> {
                        if (connectionListener != null) {
                            connectionListener.onConnectionFailed(
                                    "Failed to listen for connection: " + e.getMessage()
                            );
                        }
                    });
                }
    ...
    public void stopListening() {
        ...
        if (bluetoothSocket != null) {
            try {
                bluetoothSocket.close();
                Log.d(TAG, "Client socket closed during listening stop");
            } catch (IOException e) {
                Log.w(TAG, "Error closing client socket during listening stop", e);
            } finally {
                bluetoothSocket = null;
            }
        }
        ...
        if (serverThread != null) {
            if (serverThread.isAlive()) {
                ...
                try {
                    // Wait for the thread to die to avoid race conditions
                    serverThread.join(500); // Wait for a short period
...
//    }
        ...
        new Thread(() -> {
            ...
            while (bluetoothSocket != null && bluetoothSocket.isConnected()) {
                try {
                    ...
                    if (isDisconnecting) {
                        Log.d(TAG, "Socket closed during disconnect, stopping listening thread.");
                    } else {
    ...
    private synchronized void handleConnectionLost(String reason) {
        if (isDisconnecting) {
            Log.d(TAG, "handleConnectionLost called during disconnect, ignoring.");
            return;
        }
        ...
        // START ACTIVE RECONNECTION
        // No need for passive listening anymore, the active loop is more robust.
        if (continuousReconnectEnabled && lastConnectedDevice != null) {
            Log.d(TAG, "Connection lost, initiating active reconnection process.");
            // Remove any previously scheduled runnables to start fresh.
            reconnectionHandler.removeCallbacks(continuousReconnectRunnable);
            // Start the first reconnection attempt.
            reconnectionHandler.post(continuousReconnectRunnable);
        }
    ...
    public boolean isConnected() {
        try {
            return bluetoothSocket != null && bluetoothSocket.isConnected() && !isDisconnecting;
        } catch (Exception e) {
