    ...
    private void initializeGrid() {
        for (int i = 0; i < GridConstants.TOTAL_CELLS; i++) {
            ...
            if (col == 0 && row < GridConstants.DATA_SIZE) {
                // Row headers: 190-0 (reversed)
                cells[i].setData(String.valueOf(((GridConstants.DATA_SIZE - 1) - row) * 10));
                ...
                if (col == 0) {
                    ...
                    // Bottom column headers: 0-190
                    cells[i].setData(String.valueOf((col - 1) * 10));
                }
                cells[i].setColor(GridConstants.HEADER_COLOR);
            }
